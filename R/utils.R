...
#' data(mq)
#' dim(mq)
#' mqLong <- scp_exprsToLong(mq, f_sel = c("Raw.file", "Sequence", "Proteins"))
#' dim(mqLong)
scp_exprsToLong <- function(obj, f_sel = c("Raw.file", "sequence_charge")){
  ...
  if(ncol(pData(obj)) != 0)
    stop(paste0("No implementation for elongating 'pData(obj)'. It should be a",
                " 0-row data frame."))
  ...
  # Log process
  obj@processingData@processing <-
    c(processingData(obj)@processing, paste0("Convert data to long format: ",
                                             date()))
...
scp_exprsToWide <- function(obj){
  ...
  ed <- data.frame(seq = fd$Modified.sequence, id = id, ed)
  ...
  # Adapt feature data
  fd <- fd[!duplicated(fd$Modified.sequence), ]
  rownames(fd) <- fd$Modified.sequence
  fd <- fd[rownames(ed),]
  # Log process
  obj@processingData@processing <-
