...
63:173 contain an `unused` channel in channel 4. Our function can autmotacilly 
...
We filter out the cells that have a median RI 
...
specht2019v2 %>%
  addAssay(y = specht2019v2[["peptides"]][, sel], 
           name = "peptides:MedianFilter") %>%
  addAssayLink(from = "peptides", to = "peptides:MedianFilter", 
...
```{r}
specht2019v2 %>%
  infIsNA(i = "peptides:MedianFilter") %>%
  zeroIsNA(i = "peptides:MedianFilter") ->
  specht2019v2
```
...
This is different from the original figure. As addressed during the single cell filtering, we could not exactly reproduce 
...
longFormat(specht2019v2[, , "peptides_filt"], i = 1, colDataCols = "CellType") %>%
  ...
  p
p +
  geom_histogram(aes(x = Macrophages), fill = "grey", binwidth = 5) +
  ...
  scale_color_gradient2(low = "#048ABF", high = "#FF5733", mid = "wheat") +
  p +
