...
We also load `ggplot2`, `magrittr` and `dplyr` for convenient data manipulation
and plotting.
...
MaxQuant table provided in the SCoPE2 preprint (@Specht2019-jm). The `mqScpData`
table is a typical example of what you would get after reading in a CSV file 
using `read.csv` or `read.table`. See `?mqScpData` for more information about 
the table content. 
...
we need to provide a **metadata table** where rows contain sample information 
and columns must contain at least:
...
1. If `quantTable` is the path to a CSV file, it reads the file using 
`read.csv`. The table is converted to a `SingleCellExperiment` object. `readSCP`
needs to know in which field(s) the quantative data is stored. Those field
name(s) is/are provided by the `channelCol` field in the `metaData` table. So in
this example, the column names hodling the quantitative data in `mqScpData` are
...
is performed depending on the `batchCol` field in `quantTable`, in this case the
data will be split according to the `Set` column in `mqScpData`.
...
must also contain the `batchCol` field with batch names. 
...
clean and automatically replaces any zero in the selected quantitative data by
`NA`. 
...
Below are some examples of criteria that are used to identify low-confidence. The
...
To avoid proceeding with failed runs, another interesting filter is to remove
assays with too few features. If a batch contains less than, for example, 150 
features we can then suspect something wrong happened in that batch and it 
should be removed. Using `dims`, we can query the dimensions (hence the number 
of features and the number of samples) of all assays contained in the dataset.
...
$features \times samples \times assay$. Hence, `QFeatures` supports three-order
subsetting `x[rows, columns, assays]`. We first select the assay that have 
sufficient PSMs (the number of rows is greater than 150), and then subset the
`scp` object for the assays that meet the criterion. 
...
Note in our example all assays have sufficient number of PSMs, hence no assay 
is removed.

## Filter features based on SCP metrics

Another type of filtering is specific to SCP. In the SCoPE2 analysis, the 
authors suggest a filters based on the sample to carrier ratio (SCR), that is 
...
metadata and is accessed using the `colData`, under the `SampleType` 
field. 
...
In this dataset, `SampleType` gives the type of sample that is present in
each TMT channel. The SCoPE2 protocole includes 5 types of samples: 

* The carrier channels (`Carrier`) contain 200 cell equivalents and are 
...
* The blank channels (`Blank`) contain samples that do not contain any cell but 
are processed as single-cell samples.
* The single-cell sample channels contain the single-cell samples of interest, 
that are macrophage (`Macrophage`) or monocyte (`Monocyte`).
...
Before applying the filter, we plot the distribution of the average SCR. We can 
...
Most values are close to 0.02 as expected since the experimental ratio is 1/200.
There are a few point that have higher signal than expected. We therefore filter
out those points using a cutoff of 0.1 using again the `filterFeatures`
functions. 
...
Finally, a last PSM filter criterion is the false discovery rate (FDR) for 
...
We here divide all columns (using the regular expression wildcard `.`) by the 
...
The PSMs are aggregated over the `fcol` feature variable, here peptides. We also
need to supply an aggregating function that will tell how to combine the 
...
Then we add this assay back into the `QFeatures` object. This is done using the 
...
Here all values seems reasonable so no filtering is needed. If it were not the 
case, the same procedure as the previous section can be used for selecting the 
cells that have an associated median RI lower that a defined threshold. 
...
protein level. The computed median CVs are automatically stored in the `colData`
of the `SingleCellExperiment` assay under the `.medianCV` field
...
consistent within cells than witin the blank channels. Based on the distribution
of the blanks, we decide to keep the cells that have a median CV lower than
0.3. Note this example is inaccurate because the null distribution is based on
only 3 blank samples, but more sets could lead to a better estimation of the CV 
null distribution. Note we also keep only the samples of interest (macrophages
and monocytes) since all QC metrics are now computed.
...
transformation, the `FUN` function to apply and `STATS`, a vector of values to
apply. More conventional normalization procedure can be found in
`?QFeatures::normalize`.
...
The protein data is further processed in three steps: normalization, imputation
(using the KNN algorithm) and batch correction (using the `ComBat` algorithm).
...
The protein data contains a lot of missing values. Whether imputation is 
beneficial or deleterious for the data will not be discussed in this vignette. 
The data imputation is performed using the K nearest neighbors algorithm, with 
k = 3. This is available from the `impute` mehtod. More details about the 
arguments can be found in `?impute::impute.knn`.
...
`ComBat` is the best algorithm for batch correcting SCP data and other batch 
correcting methods could be used using the same procedure. 

We first extract the assay to process.

```{r transferColDataToAssay_proteins}
...
Because each assay contains `SingelCellExperiment` objects, we can easily apply
methods developed in the scRNA-Seq field. A useful package for dimension 
reduction on single-cell data is the `scater`.
...
`SingleCellExperiment` object for which the dimension reduction results are 
...
This is a minimalistic example with only a few plotted cells, but the original 
SCoPE2 dataset contained more than thousand cells.

## UMAP

Similarly to PCA, we can compute a UMAP using the `runUMAP` method. Note 
however that the UMAP implementation requires a initialization, usually provided
by PCA. The previous PCA results are used automatically when supplying 
`dimred = "PCA"` (`PCA` is the name of the dimension reduction result that we 
supplied in the previous section).
...
The computed UMAP can be displayed using the `plotReducedDim` function. The 
`dimred` arguments gives the name of the dimension reduction results to plot, 
here we called it `UMAP`. The samples are colored by type of sample. 
...
The UMAP plot is a very interesting plot for large datasets. A UMAP on this
small example dataset is not useful but is shown for illustration. 
...
*LMNA* (protein ID is `P02545`). A useful QC is to monitor the data 
processing at the PSM, peptide and protein level. This can easily be done thanks
to the `QFeatures` framework. Using the `subsetByFeature`, we can extract the
protein of interest and its related features in the other assays. The data is
formated to a long format table that can easily be plugged in the `ggplot2`
visualization tool. 
...
This graph helps to keep track of the data processing and highlights anomalies 
in the data. For instance, we can see that no data were recorded for 5 channels
(TMT-12 to 16). Those channels contain either macrophages or monocytes (from the
experimental design) and are expected to contain information. Thanks to this
diagnostic plot, we reported the issue to Specht and colleagues and this led to
version 3 of the SCoPE2 dataset.
