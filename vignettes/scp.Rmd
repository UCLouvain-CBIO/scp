...
metadata and is accessed using the `colData`, under the `SampleAnnotation` 
field. 
...
In this dataset, `SampleAnnotation` gives the type of sample that is present in
...
only 3 blank samples, but more sets could lead to a better estimation of the CV null distribution. Note we also keep only the samples of interest (macrophages
...
model <- model.matrix(~ SampleAnnotation, data = colData(sce))
```
...
scp %>%
  ...
  longFormat(colDataCols = c("Set", "SampleAnnotation", "Channel")) %>%
  data.frame %>%
  ...
  ## Start plotting
  ggplot(aes(x = Channel, y = value, group = rowname, col = SampleAnnotation)) +
  geom_point() +
