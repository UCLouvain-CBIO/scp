...
lower than 0.3. We also remove the blanks since all QC is performed. Note this 
example is crude since there are only 3 blank sample, but more sets could lead 
to a better estimation of the CV null distribution. 

Again, we first compute the selection criterion. Note we also keep onluy the 
samples of interest (macrophages and monocytes) since all QC metrics are now 
computed.
...
Missing data can either be imputed by replacing the missing values by predicted 
values or can be taken into account during the statistical modeling. We here 
give an example of imputation using the K nearest neighbors algorithm, with 
k = 3, that is available from the `impute` mehtod. More details about the 
...
here we called it `PCA`. The samples are colored by type of sample. 
...
SCoPE2 dataset contained over thousand cells.

## UMAP

Similarly to PCA, we can computed a UMAP using the `runUMAP` method. Note 
...
small example dataset is not useful but is shown for illustration purposes. 
...
anomalies in the data. For instance, we can see that no data were recorded for
5 samples (TMT-12 to 16). Those channels contain either macrophages or monocytes
(from the experimental design) and are expected to contain information. Thanks 
to this diagnostic plot, we reported the issue to Specht and colleagues there 
and this led to version 3 of the SCoPE2 dataset.
